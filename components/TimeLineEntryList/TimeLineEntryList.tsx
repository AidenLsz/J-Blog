...
import Styles from "./TimeLineEntryList.module.css"
import Image from "next/image"
import {navBarProps} from "@/components/NavBarView/NavBarView";

function Timeline_entrylist({navBarData}: navBarProps, {children}: IProps): JSX.Element {
    return (
                ...
                <nav role="navigation" className={Styles.list_nav}>
                    <ul className={Styles.nav_list}>
                        {navBarData.map((post: any) => (
                            <li className={`${Styles.nav_item} ${post.id == navBarData.length ? Styles.right : ""} ${post.id == 1 ? Styles.active : ""} `}
                                key={post.id}>
            ...
            <li className={Styles.item}>
                <div className={Styles.advertisement}>
                    ...
                    <div className={Styles.main}>
                        ...
                        <Image alt="广告位" width={120} height={80} className={`${Styles.lazy} ${Styles.thumb}`}
