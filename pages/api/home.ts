...
const getHomeData = (req: NextApiRequest, res: NextApiResponse): void => {
    axios.get(`${SERVERDOMAIN}/api/article-tabs`).then(tab => {
            axios.get(`${SERVERDOMAIN}/api/article-type-tabs`).then(navBarView => {
                axios.get(`${SERVERDOMAIN}/api/articles?populate=*`).then(article => {
                    axios.get(`${SERVERDOMAIN}/api/articles?sort[0]=updatedAt:desc&populate=*`).then(article_latest => {
                        axios.get(`${SERVERDOMAIN}/api/articles?sort[0]=view_count:desc&populate=*`).then(article_hot => {
